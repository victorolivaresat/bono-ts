...
export function ImportarUsuarios() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (data.errorCount === 0) {
                toast.success(`Se importaron ${data.processedCount} usuarios correctamente`)
            } else {
                toast.warning(
                    `Se importaron ${data.processedCount} usuarios. ${data.errorCount} filas con errores`
                )
            }

            // Limpiar el archivo después de la importación exitosa
            setFile(null)
            const fileInput = document.getElementById("file-upload-usuarios") as HTMLInputElement
    ...
    const downloadTemplateCSV = () => {
        const csvContent = `nombre,email,password,rol
Juan Pérez,juan.perez@example.com,password123,user
María García,maria.garcia@example.com,password456,admin`
    ...
    const downloadTemplateExcel = async () => {
        try {
            const response = await fetch("/api/usuarios/plantilla")
            ...
            <div className="space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
                        ...
                        <li><strong>password:</strong> Contraseña (requerido)</li>
                        <li><strong>rol:</strong> user o admin (requerido)</li>
                    </ul>
                    <p className="text-sm text-blue-800 mt-2">
                        <strong>Nota:</strong> Los correos deben ser únicos. Si existe un usuario con el mismo email, se omitirá.
                    </p>
                </div>
                ...
                <div className="space-y-2">
                    <label
                        htmlFor="file-upload-usuarios"
                        className="block text-sm font-medium text-gray-700"
                    >
                        Seleccionar archivo Excel o CSV
                    </label>
                    <input
                        id="file-upload-usuarios"
                        type="file"
                        accept=".xlsx,.xls,.csv"
